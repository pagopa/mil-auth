...
public class TokenGenerationTest {
    ...
    public void createTokenWithoutRefreshByPassword() {
        given()
                ...
                .formParam("username", "antonio.tarricone")
                .formParam("password", "antonio")
                .when()
                .post()
                .then()
                .statusCode(200)
    ...
    public void testCreateTokenWithWrongClientId() {
        ...
        given().contentType(MediaType.APPLICATION_FORM_URLENCODED)
                .formParam("client_id", clientId).formParam("username", "antonio.tarricone")
                .formParam("password", "antonio").formParam("grant_type", "password")
                .header("AcquirerId", ACQUIRER_ID).header("TerminalId", TERMINAL_ID)
                .header("MerchantId", MERCHANT_ID).header("RequestId", "aB1aB1aB-aB1a-aB1a-aB1a-aB1aB1aB1abb")
                .header("Channel", CHANNEL)
                .when().log().all().post()
                .then().statusCode(401)
    ...
    public void testCreateTokenWithWrongChannel() {
        ...
        given().contentType(MediaType.APPLICATION_FORM_URLENCODED)
                .formParam("client_id", CLIENT_ID).formParam("username", username).formParam("password", password)
                .formParam("grant_type", "password").header("AcquirerId", ACQUIRER_ID).header("TerminalId", TERMINAL_ID)
                .header("MerchantId", MERCHANT_ID).header("RequestId", "aB1aB1aB-aB1a-aB1a-aB1a-aB1aB1aB1abb")
                .header("Channel", channel)
                .when()
                .log()
    ...
    public void testCreateTokenWithWrongUser() {
        ...
        given().contentType(MediaType.APPLICATION_FORM_URLENCODED)
                .formParam("client_id", CLIENT_ID).formParam("username", username).formParam("password", password)
                .formParam("grant_type", "password").header("AcquirerId", ACQUIRER_ID)
                .header("TerminalId", TERMINAL_ID)
                .header("MerchantId", MERCHANT_ID)
