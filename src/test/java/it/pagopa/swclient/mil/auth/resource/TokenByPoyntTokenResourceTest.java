...
import static it.pagopa.swclient.mil.auth.bean.JsonPropertyName.REFRESH_TOKEN;
...
class TokenByPoyntTokenResourceTest {
	...
	void testOk() {
		...
		given()
			...
			.formParam(FormParamName.GRANT_TYPE, POYNT_TOKEN)
			.formParam(FormParamName.EXT_TOKEN, EXT_TOKEN)
			.formParam(FormParamName.ADD_DATA, ADD_DATA)
			.formParam(FormParamName.SCOPE, OFFLINE_ACCESS)
			.when()
			.post()
			.then()
			.statusCode(200)
	...
	void testWebApplicationExceptionGettingBusinessObject() {
		...
		given()
			...
			.statusCode(401)
			.contentType(MediaType.APPLICATION_JSON)
			.body(ERRORS, hasItem(EXT_TOKEN_NOT_VALID));
	}
	...
	void testExceptionGettingBusinessObject() {
		...
		given()
			...
			.statusCode(500)
			.contentType(MediaType.APPLICATION_JSON)
			.body(ERRORS, hasItem(ERROR_VALIDATING_EXT_TOKEN));
	}
